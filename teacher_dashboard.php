...
// --- หาค่า teacher_id จาก session ---
$teacherId = (int)($_SESSION['user']['user_id'] ?? $_SESSION['user']['id'] ?? 0);

$tid = current_user_id(); // id ครูที่ล็อกอิน
...
// จำนวนนักเรียนที่ลงทะเบียน (active) ในคอร์สของครู
$st = $pdo->prepare("
...
// จำนวนงานที่มอบหมายของครู (อิง assignments.teacher_id ตรง ๆ)
$st = $pdo->prepare("SELECT COUNT(*) FROM assignments WHERE teacher_id = ?");
$st->execute([$tid]);
$totalAssignments = (int)$st->fetchColumn();

// 5 การส่งล่าสุดของนักเรียนในงานของครู
$recent = $pdo->prepare("
  SELECT s.submission_id, s.submitted_at, s.score,
    ...
    <div class="card">
      ...
      <div class="kpis">
        <div class="kpi">
          ...
          <div class="v"><?= $totalCourses ?></div>
        </div>
        <div class="kpi">
          <div class="h">จำนวนนักเรียนรวม</div>
          <div class="v"><?= $totalStudents ?></div>
        </div>
        <div class="kpi">
          <div class="h">งานที่มอบหมาย</div>
          <div class="v"><?= $totalAssignments ?></div>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <table class="table">
        ...
        <tbody>
          ...
          if (!$recentRows): ?>
            <tr>
              <td colspan="6" class="muted">ยังไม่มีการส่ง</td>
            </tr>
          <?php endif; ?>
        </tbody>
      </table>
