...
if (in_array($roleF, ['student', 'teacher', 'admin'])) {
...
// รายการผู้ใช้ (จัดหน้า)
$sql = "SELECT user_id, name, email, role, created_at, updated_at
        FROM users WHERE 1";
$args = [];
if ($q !== '') {
...
<body>
    ...
    <div class="sidebar">
        ...
        <a href="teacher_assign_list.php" class="<?= $current === 'teacher_assign_list.php' ? 'active' : '' ?>"><i class="bi bi-card-checklist"></i> งานครู</a>
                    ...
                    <?php if ($pages > 1): ?>
                        <div class="pager">
                            <?php for ($i = 1; $i <= $pages; $i++):
                                $qs = http_build_query(array_filter(['q' => $q, 'role' => $roleF, 'page' => $i]));
                            ?>
                                <?php if ($i === $page): ?>
